...
namespace stok.Repository.Data.UserAccount
{
    public class UserAccountData(DatabaseContext context) : BaseData(context), IUserAccountData
    {
        ...
        {
            var user =  await BaseQuery<user.UserAccount>(false).Include(u => u.UserInformation).Include(p => p.PositionType).FirstOrDefaultAsync(x => x.Email == email, cancellation);
            return user;
        }

        public async Task<user.UserAccount> GetUserByEmailWithTracking(string email, CancellationToken cancellation = default)
        {
            var user = await BaseQuery<user.UserAccount>(true).Include(u => u.UserInformation).Include(p => p.PositionType).FirstOrDefaultAsync(x => x.Email == email, cancellation);
            return user;
        }
        ...
        {
            var user = await BaseQuery<user.UserAccount>(false).Include(u => u.UserInformation).Include(p => p.PositionType).FirstOrDefaultAsync(x => x.Id == userId, cancellation);
            return user;
        }

        public async Task<user.UserAccount> GetUserByIdWithTracking(int userId, CancellationToken cancellation = default)
        {
            var user = await BaseQuery<user.UserAccount>(true).Include(u => u.UserInformation).Include(p => p.PositionType).FirstOrDefaultAsync(x => x.Id == userId, cancellation);
            return user;
        }
        ...
        {
            var user = await BaseQuery<user.UserAccount>(true).Include(x => x.UserInformation).Include(p => p.PositionType).FirstOrDefaultAsync(x => x.Id == userId, cancellation);
            return user;
        }
        ...
        {
            var user = await BaseQuery<user.UserAccount>(false).Include(x => x.UserInformation).Include(p => p.PositionType).FirstOrDefaultAsync(x => x.Id == userId, cancellation);
            return user;
        }
